...
const fetchCourses = async () => {
  try {
    const res = await axios.get(`/api/course/list?teacherId=${teacherId}`)
...
const handleSubmit = async () => {
  ...
  try {
    const url = showEditModal.value ? '/api/course/update' : '/api/course/add'
    const method = showEditModal.value ? 'put' : 'post'
    const payload = showEditModal.value
      ? { id: form.value.id, courseName: form.value.courseName, description: form.value.description }
...
const goToResource = (courseId) => {
  router.push(`/course-resource/${courseId}`)
}
